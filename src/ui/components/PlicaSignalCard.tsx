...
export function PlicaSignalCard({
  ...
  const health = data.unavailable
    ? "red"
    : derivePaneHealth(data.summary, data.attention);
  ...
  const itemsIn = (key: string) =>
    (data.attention?.items ?? [])
      .filter(
        (item) =>
          !item.dismissal && attentionGroupFor(item.sourceKind)?.key === key,
      )
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <span
          ...
          className={cn(
            ...
            PLICA_HEALTH_DOT_CLASSES[health],
          )}
        />
        <span
          data-signal-total
          ...
          {cells.map((cell) => {
            ...
            return (
              <Popover
                key={cell.key}
                open={isOpen}
                onOpenChange={(next) => setOpenGroup(next ? cell.key : null)}
              >
                ...
                {/* The list floats rather than expanding the card. A pinned
                    company sits in a grid row with its siblings; growing one
                    of them re-flows the bar and shoves the workspace down,
                    which is a lot of movement for a glance. */}
                <PopoverContent
                  align="start"
                  sideOffset={6}
                ...
                >
                  ...
                  <ul className="max-h-72 space-y-0.5 overflow-y-auto">
                    {itemsIn(cell.key).map((item) => (
                      <PlicaAttentionLedgerRow
                        key={item.id}
                        item={item}
                        company={company}
                        nowMs={nowMs}
